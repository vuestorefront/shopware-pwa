      ...
      <SfTable v-else class="orders">
        ...
        <SfTableRow v-for="order in orderList" :key="order[0]">
          <SfTableData v-for="(data, key) in order" :key="key">
            <template v-if="key === 3"><!-- order status -->
              <span
                :class="{
            ...
            <template v-else-if="key === 2"><!-- order date -->
              {{ formatDate(data) }}
            </template>
            <template v-else-if="key === 1"><!-- total amount -->
              {{ formatPrice(data) }}
            </template>
...
export default {
  name: "OrderHistory",
  components: {SfList, SfDivider, SfTable, SfButton},
  props: {
  },
  data() {
    return {
  ...
  computed: {
    orderList() {
      return this.orders && this.orders.map(({orderNumber, amountTotal, orderDateTime, stateMachineState}) => [orderNumber, amountTotal, orderDateTime, stateMachineState.name])
    }
  },
  methods: {
    formatDate(date) {
